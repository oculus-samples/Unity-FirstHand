...
namespace Oculus.Interaction.ComprehensiveSample
{
    ...
    /// </summary>
    public class AnimatedGhostHandControllers : ActiveStateObserver
    {
        ...
        [SerializeField] private List<MaterialPropertyBlockEditor> _materialPropertyBlockEditors = new List<MaterialPropertyBlockEditor>();
        ...
        private void SetAlphaValue(float alpha)
        {
            ...
            {
                var renderers = _materialPropertyBlockEditors[i].Renderers;

                for (int j = 0; j < renderers.Count; j++)
                    renderers[j].enabled = enableRenderers;

                if (enableRenderers)
                {
                    _materialPropertyBlockEditors[i].MaterialPropertyBlock.SetFloat(_opacityShaderParameterId, alpha);
                    _materialPropertyBlockEditors[i].MaterialPropertyBlock.SetFloat(_outlineOpacityShaderParameterId, alpha);
                }
            }
        }
